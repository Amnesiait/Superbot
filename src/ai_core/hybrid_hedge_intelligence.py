...
from src.constants import VolatilityAdjustment
...
class HedgeDecision:
    ...
    size_multiplier: float = 1.0 # Multiplier applied to base hedge size
...
class HybridHedgeIntelligence:
    ...
    def analyze_hedge_decision(
        ...
        base_hedge = self._calculate_base_hedge(positions, market_data, drawdown_pct)
        ...
        return HedgeDecision(
            ...
            factors=factors,
            timing=timing,
            size_multiplier=round(final_hedge / base_hedge, 2) if base_hedge > 0 else 1.0
        )
    ...
    def _analyze_volatility(self, market_data: Dict) -> float:
        ...
        # Adaptive scaling
        if volatility_ratio > VolatilityAdjustment.EXTREME_THRESHOLD:
            # Very high volatility (30%+ above normal)
            factor = VolatilityAdjustment.EXTREME_REDUCTION
            logger.info(f"[VOLATILITY] Very high ({volatility_ratio:.2f}x) → Reduce hedge {round((1-VolatilityAdjustment.EXTREME_REDUCTION)*100)}%")
        elif volatility_ratio > VolatilityAdjustment.HIGH_THRESHOLD:
            # High volatility (10-30% above normal)
            factor = VolatilityAdjustment.HIGH_REDUCTION
            logger.info(f"[VOLATILITY] High ({volatility_ratio:.2f}x) → Reduce hedge {round((1-VolatilityAdjustment.HIGH_REDUCTION)*100)}%")
        elif volatility_ratio < VolatilityAdjustment.LOW_THRESHOLD:
            # Low volatility (20%+ below normal)
            factor = VolatilityAdjustment.LOW_INCREASE
            logger.info(f"[VOLATILITY] Low ({volatility_ratio:.2f}x) → Increase hedge {round((VolatilityAdjustment.LOW_INCREASE-1)*100)}%")
        else:
            # Normal volatility
            factor = 1.0
    ...
    def _analyze_time_decay(self, positions: List[Dict]) -> float:
        ...
        if time_in_loss_minutes < 3:
            ...
            factor = 0.8
            logger.debug(f"[TIME_DECAY] Fresh loss ({time_in_loss_minutes:.1f}m) → Reduce hedge 20%")
        elif time_in_loss_minutes < 10:
            # Fresh loss (3-10 min)
            factor = 0.9
            logger.debug(f"[TIME_DECAY] Recent loss ({time_in_loss_minutes:.1f}m) → Reduce hedge 10%")
        elif time_in_loss_minutes > 30:
            # Old loss (>30 min)
            factor = 1.15
            logger.debug(f"[TIME_DECAY] Extended loss ({time_in_loss_minutes:.1f}m) → Increase hedge 15%")
        else:
    ...
    def _analyze_oracle(self, oracle, positions: List[Dict], market_data: Dict) -> float:
        ...
        try:
            ...
            # [PHASE 2 ENHANCEMENT] Logic Update
            # If Oracle SUPPORTS the hedge direction with High Confidence, we Boost.
            # If Oracle OPPOSES the hedge direction, we Reduce/Block.
            
            oracle_supports = False
            if hedge_direction == "BUY" and oracle_pred == "UP":
                oracle_supports = True
            elif hedge_direction == "SELL" and oracle_pred == "DOWN":
                oracle_supports = True
            ...
            if oracle_supports:
                if oracle_conf > 0.85:
                    factor = 1.25 # Boost 25% (High Conviction)
                    logger.info(f"[ORACLE] Strong Support: {oracle_pred} ({oracle_conf:.0%}) supports {hedge_direction} → Boost 25%")
                elif oracle_conf > 0.70:
                    factor = 1.10 # Boost 10%
                    logger.debug(f"[ORACLE] Support: {oracle_pred} ({oracle_conf:.0%}) supports {hedge_direction} → Boost 10%")
    ...
    def _calculate_base_hedge(self, positions: List[Dict], market_data: Dict, drawdown_pct: float = 0.0) -> float:
        """

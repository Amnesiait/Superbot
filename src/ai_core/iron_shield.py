...
class IronShield:
    ...
    def calculate_dynamic_risk_budget(self, equity, balance, oracle_confidence=0.0):
        """
        DETERMINISTIC RULE-BASED SYSTEM (No Hallucinations Possible)
        Allocates risk budget based on MATH, not guesses.
        
        Tiers:
        1. Normal (0-5% DD): Conservative, 2.5x max multiplier
        2. Recovery (5-10% DD): Moderate, 3.0x max multiplier
        3. GOD MODE (10-15% DD): Aggressive, 5.0x max multiplier
        4. VALKYRIE (15%+ DD): Max Power, 10.0x max multiplier
        """
        if balance <= 0: return 0.0, 1.0, 1
            
        drawdown_pct = (balance - equity) / balance
        
        # Default: Tier 1 (Normal)
        tier = 1
        multiplier = 2.5
        
        if drawdown_pct < 0.05:
            # Tier 1: Normal Trading
            # Strict safety, limited martingale
            tier = 1
            multiplier = 2.5
            
        elif drawdown_pct < 0.10:
            # Tier 2: Light Recovery
            # Validation: We need slightly more room to breathe
            tier = 2
            multiplier = 3.0
            
        elif drawdown_pct < 0.15:
            # Tier 3: GOD MODE (The AI Unlock)
            # This solves the "$10k loss" problem where AI was blocked
            tier = 3
            multiplier = 5.0
            
            # Intelligence Bonus: If Oracle is SUPER confident (>85%), allow more
            if oracle_confidence > 0.85:
                multiplier = 6.0
                
        else:
            # Tier 4: VALKYRIE (Emergency)
            # We are fighting for survival. Unleash the math.
            tier = 4
            multiplier = 10.0
    ...
    def calculate_recovery_lot_size(self, 
        ...
        # 5. Safety Clamp - TIERED SYSTEM INTEGRATION
        # We calculate the effective Tier based on the 'positions' state.
        # If we have many positions, we are likely in drawdown.
        ...
        if position_count >= 3:
            ...
            max_multiplier = 5.0
        if position_count >= 6:
            # Tier 4 territory (Emergency)
            max_multiplier = 10.0
            
        # AI Confidence Boost
        if aggression_multiplier > 1.1: # AI is confident (RSI confirmation)
            max_multiplier += 1.0
            
        max_allowed = initial_lot * max_multiplier
        
        # Hard Ceiling for any single trade (e.g., 50 lots)
        # Prevents "Fat Finger" math errors
        ABSOLUTE_MAX_LOT = 50.0 
        max_allowed = min(max_allowed, ABSOLUTE_MAX_LOT)
    ...
    def calculate_defense(self, current_loss_lot, spread_points, atr_value=0.0, trend_strength=0.0, 
        ...
        # SAFETY: Cap the Hedge Lot using DETERMINISTIC TIERS
        # Get dynamic multiplier based on account health (drawdown)
        
        base_mult = 2.5
        
        # Use simple logic mapped to the tiers we defined
        if drawdown_pct > 0:
            if drawdown_pct >= 0.15: base_mult = 10.0 # Tier 4
            elif drawdown_pct >= 0.10: base_mult = 5.0 # Tier 3 (God Mode)
            elif drawdown_pct >= 0.05: base_mult = 3.0 # Tier 2
        else:
            # Fallback for when drawdown isn't passed (use hedge level proxy)
            if hedge_level >= 3: base_mult = 5.0
            elif hedge_level == 2: base_mult = 3.0
            
        # Oracle Boost (Confidence Check) - PHASE 2 ENHANCEMENT
        # If Oracle is highly confident (>85%), we unlock a small bonus multiplier
        # This is "Smart Aggression"
        oracle_bonus = 0.0
        
        # We need to access Oracle confidence here. It's passed as 'aggression_score' 
        # (which is a proxy in current code) or we need to update signature.
        # Actually, let's use the 'oracle_prediction' and 'aggression_multiplier' if available?
        # The calling code passes 'aggression_score' which maps to RSI/Oracle.
        
        # Let's trust the 'aggression_score' which flows from hybrid intelligence (Layer 5)
        # If aggression_score > 1.1, it means HIGH CONFIDENCE from Hybrid system.
        
        if aggression_score > 1.2: # Very High Confidence
            oracle_bonus = 1.0     # Allow +1.0x (e.g. 2.5x -> 3.5x)
        elif aggression_score > 1.1: # High Confidence
            oracle_bonus = 0.5     # Allow +0.5x
            
        # Apply bonus
        base_mult += oracle_bonus
        
        # VOLATILITY PENALTY (Phase 2)
        # If volatility is extreme (>3.0x), reduce multiplier for safety
        if volatility_ratio > 3.0:
            base_mult *= 0.8  # Reduce by 20%
            logger.info(f"[IRON SHIELD] Extreme Volatility ({volatility_ratio:.1f}x) -> Penalty applied (0.8x)")

        max_hedge = current_loss_lot * base_mult
        
        if hedge_lot > max_hedge:
            hedge_lot = max_hedge
            tier_name = "TIER 1"
            if base_mult >= 10: tier_name = "VALKYRIE"
            elif base_mult >= 5: tier_name = "GOD MODE"
            
            logger.info(f"[IRON SHIELD] Cap Hit ({tier_name}): Math={hedge_lot:.2f} > Cap={max_hedge:.2f} (Mult {base_mult:.1f}x)")
            hedge_lot = max_hedge

...
class RangeWorker(BaseWorker):
    ...
    def get_signal(self, market_data: Dict[str, Any]) -> Tuple[str, float, str]:
        ...
        if rsi < 50:
            # Only Buy Low if NOT overlapping with strong downtrend logic
            if rsi < 30: # Strong Downtrend risk - be careful
                confidence = 0.5 # Lower confidence
                reason = f"Oversold Bounce Attempt (RSI {rsi:.1f})"
            else:
            ...
            # Only Sell High if NOT overlapping with strong uptrend logic
            if rsi > 70: # Strong Uptrend risk - be careful
                confidence = 0.5 # Lower confidence
                reason = f"Overbought Rejection Attempt (RSI {rsi:.1f})"
            else:

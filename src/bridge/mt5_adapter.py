...
class MT5Adapter(BrokerAdapter):
    ...
    def execute_order(self, symbol, action, volume, order_type, price=None, sl=0.0, tp=0.0, magic=0, comment="", ticket=None, **kwargs) -> Dict:
        ...
        # [QUANTUM SNIPER] Enhanced Order Type Mapping
        if order_type == "BUY":
            ...
            logger.error(f"[MT5] Unknown order type: {order_type}")
        ...
        request = {
            ...
            "type": mt5_type,
            "price": price if price else (mt5.symbol_info_tick(symbol).ask if "BUY" in order_type else mt5.symbol_info_tick(symbol).bid),
            "sl": sl,
            "tp": tp,
        ...
        # [QUANTUM SNIPER] Pending Orders require different filling mode
        if "LIMIT" in order_type or "STOP" in order_type:
            request["type_filling"] = mt5.ORDER_FILLING_RETURN # Pending orders usually use RETURN
    ...
    def close_hedge_by_ticket(self, ticket: int, opposite_ticket: int, symbol: str, volume: float) -> dict:
        """
        Executes ORDER_TYPE_CLOSE_BY to save spread costs.
        Closes 'ticket' using 'opposite_ticket'.
        """
        try:
            # Ensure volume is normalized
            volume = self.normalize_lot_size(symbol, volume)
            
            # CRITICAL FIX: CLOSE_BY is an ACTION, not an ORDER TYPE
            # Previous implementation caused "Invalid price (10015)" errors
            request = {
                "action": mt5.TRADE_ACTION_CLOSE_BY,  # CORRECT: Use CLOSE_BY action
                "symbol": symbol,
                "position": ticket,  # Position to close
                "position_by": opposite_ticket,  # Close it BY this opposite position
                "comment": "CloseBy Opt"[:31],  # MT5 limits comment to 31 chars
            }
            # Note: TRADE_ACTION_CLOSE_BY does NOT need:
            # - volume (MT5 calculates the matched amount automatically)
            # - type (not applicable for CLOSE_BY action)
            # - price (MT5 determines this automatically - no market execution)
            # - type_time/type_filling (not applicable)
            
            # Debug log
            logger.debug(f"[CLOSE_BY] Request: Closing #{ticket} by #{opposite_ticket} (Vol: {volume})")
            
            result = mt5.order_send(request)
            
            if result is None:
                logger.error(f"[CLOSE_BY] Order send returned None for #{ticket}")
                return {"retcode": -1, "comment": "Order send failed (result is None)"}
                
            if result.retcode != mt5.TRADE_RETCODE_DONE:
                logger.warning(f"[CLOSE_BY] Failed: {result.comment} ({result.retcode})")
            else:
                logger.debug(f"[CLOSE_BY] Success: Closed #{ticket} by #{opposite_ticket} (Vol: {volume}) spread saved!")
                
            return result._asdict()
            
        except Exception as e:
            logger.error(f"[CLOSE_BY] Exception: {e}")

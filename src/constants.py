...
SYSTEM_VERSION: Final[str] = "7.0.0"  # Production Release: Complete Cleanup + P95 Calibration + Single-Instance Enforcement (Jan 26, 2026)
SYSTEM_FULL_NAME: Final[str] = "Adaptive Evolution Trading & Hedging Execution Robot"


## ============================================================================
# Trading Universe (Sincronizado con Auditoría MT5)
# ============================================================================

# Nombres limpios, SIN barras, tal como sugiere el auditor.
PRIME_CRYPTO: Final[list] = ["BTCUSD", "ETHUSD"] 

PRIME_FOREX: Final[list] = ["EURUSD", "GBPUSD", "USDJPY", "USDCHF", "AUDUSD", "USDCAD", "NZDUSD"]

PRIME_COMMODITIES: Final[list] = ["XAUUSD", "XAGUSD"] 

# Dejamos esto vacío por seguridad, ya que el auditor no encontró los índices estándar.
PRIME_INDICES: Final[list] = []
# ============================================================================
...
class RiskLimits:
    ...
    MAX_POSITIONS_PER_SYMBOL: Final[int] = 8  # Bucket limit (Increased to 8)
    MAX_HEDGES_PER_SYMBOL: Final[int] = 6  # [CONSOLIDATED] Max hedges per symbol (base + 6 hedges = 7 total)
    MAX_GLOBAL_POSITIONS: Final[int] = 8  # [CONSOLIDATED] Max positions globally
    MAX_GLOBAL_POSITIONS_WITH_GOD_MODE: Final[int] = 14  # With God Mode buffer (+6)
...
class ZoneRecoveryConfig:
    ...
    MIN_AGE_SECONDS: Final[float] = 2.0  # Reduced from 3.0
    HEDGE_COOLDOWN_SECONDS: Final[float] = 5.0  # Reduced from 15.0 for faster reaction
    BUCKET_CLOSE_COOLDOWN_SECONDS: Final[float] = 5.0  # Reduced from 15.0
...
class VolatilityAdjustment:
    """Volatility adjustment factors for hedging."""
    EXTREME_THRESHOLD: Final[float] = 1.3
    HIGH_THRESHOLD: Final[float] = 1.1
    LOW_THRESHOLD: Final[float] = 0.8
    
    EXTREME_REDUCTION: Final[float] = 0.7   # Reduce hedge by 30%
    HIGH_REDUCTION: Final[float] = 0.85     # Reduce hedge by 15%
    LOW_INCREASE: Final[float] = 1.15       # Increase hedge by 15%


class AdaptiveRisk:
    """Configuration for dynamic volatility adaptation."""
    # Volatility Multipliers (Current ATR / Baseline ATR)
    VOL_LOW: Final[float] = 0.8
    VOL_HIGH: Final[float] = 1.5
    VOL_EXTREME: Final[float] = 3.0
    
    # Cooldown Scaling
    COOLDOWN_MIN: Final[float] = 2.0   # Minimum cooldown (seconds)
    COOLDOWN_BASE: Final[float] = 5.0  # Base cooldown from ZoneRecoveryConfig
    COOLDOWN_MAX: Final[float] = 30.0  # Maximum cooldown
    
    # Zone Scaling
    ZONE_BASE: Final[float] = 15.0      # Base zone pips (Gold)
    ZONE_MAX: Final[float] = 50.0       # Maximum zone pips
...
class AssetProfiles:
    ...
    GOLD: Final[int] = 15  # Tightened from 25 for precision scalping
    SILVER: Final[int] = 30
    INDEX: Final[int] = 50
    BTC: Final[int] = 100
    ETH: Final[int] = 50
...
# ============================================================================
# Profit Buffer & Scaling Constants
# ============================================================================

class ProfitBuffer:
    """Profit buffer scaling based on position count."""
    # Buffer scaling: progressive increase with more hedges
    BUFFER_1_2_POSITIONS: Final[float] = 0.25    # Single + 1 hedge: minimal buffer
    BUFFER_3_4_POSITIONS: Final[float] = 0.50    # 2-3 hedges: standard buffer
    BUFFER_5_8_POSITIONS: Final[float] = 1.00    # 4-7 hedges: moderate buffer
    BUFFER_9PLUS_POSITIONS: Final[float] = 1.50  # 8+ hedges: larger buffer (emergency mode)
    
    # Emergency exit buffer (override all other logic)
    EMERGENCY_EXIT_BUFFER: Final[float] = 0.10   # Nearly zero buffer for emergency closes
...
class TimeThresholds:
    """Time-based exit thresholds for different position counts."""
    # Deep hedge (10+ positions) time triggers
    DEEP_HEDGE_ESCAPE_MINUTES: Final[int] = 45        # Force exit after 45 minutes
    DEEP_HEDGE_DECAY_MINUTES: Final[int] = 20         # Apply target decay after 20 minutes
    DEEP_HEDGE_ESCAPE_TARGET: Final[float] = 0.50     # Escape with minimal profit ($0.50)
    DEEP_HEDGE_DECAY_MULTIPLIER: Final[float] = 0.25  # Reduce target to 25% after 20m
    
    # Medium hedge (3-9 positions) time triggers
    MED_HEDGE_ESCAPE_MINUTES: Final[int] = 90         # Force exit after 90 minutes
    MED_HEDGE_ESCAPE_TARGET: Final[float] = 0.50      # Escape with minimal profit ($0.50)
    
    # Local hedge cap (number of hedges before time decay activates)
    LOCAL_HEDGE_CAP: Final[int] = 5  # Raised from 3 to 5 for better hedge strategy

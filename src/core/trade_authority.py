...
class TradeAuthority:
    ...
    def __init__(self, config=None):
        ...
        # --- THE CONSTITUTION (Immutable Laws) ---
        self.max_global_positions_base = 8 # [RESTORED] STRICT GLOBAL CAP (Increased to 8)
        self.max_hedges_per_symbol = 6 # [RESTORED] STRICT SYMBOL CAP (Increased to 6)
        self.god_mode_buffer = 6 # [GOD MODE] Emergency allowance: +6 positions (Max 12 total)
        ...
        logger.info("[SUPREME COURT] Trade Authority Initialized. The Law is active (Strict Limits).")
    ...
    def check_constitution(self, broker, symbol: str, volume: float, action: str, is_god_mode: bool = False) -> Tuple[bool, str]:
        ...
        # GOD MODE gets +3 buffer on global cap too
        ...
        if total_positions >= effective_global_cap:
            msg = f"VETOED: Global Cap Reached ({total_positions}/{effective_global_cap}). State: {self.volatility_state}"
        ...
        # GOD MODE gets +3 buffer (6 normal + 3 GOD MODE = 9 max)
        ...
        if symbol_positions >= effective_symbol_cap:
            mode_label = "[GOD MODE EMERGENCY]" if is_god_mode else ""
            msg = f"VETOED: Symbol Cap Reached {mode_label} for {symbol} ({symbol_positions}/{effective_symbol_cap}). State: {self.volatility_state}"
        ...
        # Log approval for GOD MODE trades (important for monitoring)
        if is_god_mode:
            logger.info(f"[SUPREME COURT] ✅ GOD MODE APPROVED: {symbol} position #{symbol_positions + 1}/{effective_symbol_cap}")

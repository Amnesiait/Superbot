...
class MarketDataManager:
    ...
    def get_contract_size(self, symbol: str) -> float:
        """
        Get the contract size for a symbol from the broker.
        Falls back to standard defaults if API fails.
        """
        try:
            # Try to fetch from MT5 directly if strictly needed
            ...
            logger.warning(f"[MARKET_DATA] Failed to fetch contract size for {symbol}: {e}")
            # Ultimate fallback

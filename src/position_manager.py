...
from .constants import ProfitBuffer, TimeThresholds
...
class PositionManager:
    ...
    def _record_close_slippage_sample(self, symbol: str, request_price: float, fill_price: float) -> None:
        """
        Record slippage sample from a closed trade for P95 calibration.
        Slippage = actual fill price - requested price (in pips/units).
        Uses rolling window to maintain recent history.
        """
        if not self._slippage_enabled or not symbol:
            return
        
        try:
            # Calculate absolute slippage in USD per lot
            # For most symbols, slippage is the absolute difference divided by lot size
            # This is a generic calculation - symbol-specific adjustments can be added
            ...
            # Slippage is the absolute difference between requested and filled price
            slippage_amount = abs(fill_price - request_price)
            
            # Convert to USD per lot (standard lot = 100,000 for most forex)
            # For gold (XAUUSD), convert oz difference to USD equivalent
            if "XAU" in symbol or "GOLD" in symbol:
                # Gold is quoted per oz, 1 lot = 100 oz
                slippage_usd_per_lot = slippage_amount * 100.0
            else:
                # Forex: convert pips to USD
                # Standard: 1 pip = 0.0001, 1 lot = 100,000 units
                # So 1 pip on 1 lot = $10 (for pairs vs USD)
                slippage_usd_per_lot = slippage_amount * 100000.0 * 10.0
            
            # Record sample with thread safety
            with self._lock:
                if symbol not in self._slippage_samples_per_lot_usd:
                    self._slippage_samples_per_lot_usd[symbol] = collections.deque(maxlen=self._slippage_window)
                
                self._slippage_samples_per_lot_usd[symbol].append(slippage_usd_per_lot)
                sample_count = len(self._slippage_samples_per_lot_usd[symbol])
            
            # Log at debug level to avoid noise
            if sample_count % 10 == 0:  # Log every 10 samples
                p95_val = self._get_slippage_p95_per_lot_usd(symbol)
                logger.debug(
                    f"[SLIPPAGE CALIBRATION] {symbol}: recorded sample {slippage_usd_per_lot:.2f} USD/lot, "
                    f"total_samples={sample_count}, p95={p95_val:.2f}"
                )
        except Exception as e:
            logger.debug(f"[SLIPPAGE CALIBRATION] Failed to record sample for {symbol}: {e}")
    ...
    async def execute_calculated_recovery(self, broker, bucket_id: str, market_data: Dict, shield=None) -> bool:
        ...
        with self._lock:
            ...
            # [CRITICAL ENHANCEMENT] GOD MODE ML-BASED TREND EXIT
            # Prevent death spirals by detecting sustained trends BEFORE averaging down
            # This would have prevented the 8-position BUY loss in downtrend
            god_mode_ml_enabled = str(os.getenv("AETHER_GOD_MODE_ML_ENABLED", "1")).strip().lower() in ("1", "true", "yes", "on")
            
            if god_mode_ml_enabled and candles and len(candles) >= 20:
                # Use Oracle for regime detection
                if not hasattr(self, '_god_mode_oracle'):
                    from src.ai_core.oracle import Oracle
                    # [FIX] Lazy init Oracle with available dependencies
                    try:
                        self._god_mode_oracle = Oracle(
                            mt5_adapter=getattr(self, 'broker_adapter', None),
                            tick_analyzer=getattr(self, 'tick_analyzer', None),
                            global_brain=getattr(self, 'global_brain', None)
                        )
                    except Exception as oracle_init_err:
                        logger.error(f"[GOD MODE] Failed to initialize Oracle: {oracle_init_err}. Disabling trend detection.")
                        god_mode_ml_enabled = False
                
                if god_mode_ml_enabled:
                    try:
                        # Detect market regime
                        regime, signal_direction = self._god_mode_oracle.get_regime_and_signal(candles)
                        
                        # CRITICAL SAFETY: Exit if fighting sustained trend
                        if net_vol > 0 and regime == "TREND_DOWN":
                            # Long positions in downtrend = death spiral
                            logger.critical(f"🚨 [GOD MODE] TREND EXIT: Sustained DOWNTREND detected")
                            logger.info(f"[GOD MODE] Exiting {len(positions)} long positions. Deficit: -${deficit:.2f}")
                            
                            # Close all positions immediately
                            close_result = await self.close_bucket_positions(
                                broker,
                                bucket_id,
                                symbol,
                                trace={"reason": "GOD_MODE_TREND_EXIT_DOWN", "confidence": 0.85, "emergency": True}
                            )
                            
                            if close_result:
                                logger.info(f"✅ [GOD MODE] Trend exit successful. Prevented death spiral.")
                                return True  # Recovery "successful" - positions are closed
                            else:
                                # Close failed - buffer check blocked it. Open SHORT hedge instead to catch bounce
                                logger.warning(f"[GOD MODE] Could not close long positions. Opening SHORT hedge to catch downtrend.")
                                action = "SELL"
                                # Continue to recovery signal logic (don't return, proceed with hedge opening)
                        
                        elif net_vol < 0 and regime == "TREND_UP":
                            # Short positions in uptrend = death spiral
                            logger.critical(f"🚨 [GOD MODE] TREND EXIT: Sustained UPTREND detected")
                            logger.info(f"[GOD MODE] Exiting {len(positions)} short positions. Deficit: -${deficit:.2f}")
                            
                            close_result = await self.close_bucket_positions(
                                broker,
                                bucket_id,
                                symbol,
                                trace={"reason": "GOD_MODE_TREND_EXIT_UP", "confidence": 0.85, "emergency": True}
                            )
                            ...
                            if close_result:
                                ...
                                # Close failed - buffer check blocked it. Open LONG hedge instead to catch rally
                                logger.warning(f"[GOD MODE] Could not close short positions. Opening LONG hedge to catch uptrend.")
                                action = "BUY"
                                # Continue to recovery signal logic (don't return, proceed with hedge opening)
                        
                        elif regime == "RANGE":
                            # Safe to average down in ranging market
                            logger.debug(f"[GOD MODE] Market in RANGE - safe to proceed with recovery")
                    
                    except RecursionError as re:
                        # Recursion depth exceeded in Oracle evaluation - skip trend detection this cycle
                        logger.warning(f"[GOD MODE] Recursion depth exceeded in trend detection: {re}. Proceeding without trend check.")
                        # Reset Oracle and disable for this cycle to prevent cascade
                        if hasattr(self, '_god_mode_oracle'):
                            delattr(self, '_god_mode_oracle')
                        god_mode_ml_enabled = False
                    
                    except Exception as e:
                        # Failsafe: if detection fails, log but don't block
                        logger.warning(f"[GOD MODE] Trend detection error: {e}. Proceeding with caution.")
                        # Reset Oracle and disable for this cycle
                        if hasattr(self, '_god_mode_oracle'):
                            delattr(self, '_god_mode_oracle')
            ...
            if not signal:
                 # [FIX] Add attempt counter to prevent infinite recovery loops
                 if not hasattr(stats, 'recovery_attempt_count'):
                     stats.recovery_attempt_count = 0
                 if not hasattr(stats, 'last_recovery_attempt_time'):
                     stats.last_recovery_attempt_time = 0.0
                 
                 time_since_last_attempt = time.time() - stats.last_recovery_attempt_time
                 
                 # Check if deficit is critical (30% equity - conservative)
                 if deficit > (acct_equity * 0.30):
                     ...
                     if self.trade_authority and hasattr(self.trade_authority, 'current_global_cap'):
                         ...
                         if current_count >= max_cap:
                             # [FIX] Check if we've been stuck for too long
                             if time_since_last_attempt < 300 and stats.recovery_attempt_count >= 5:
                                 # Escape: Been blocked for 5+ attempts in 5 mins - give up and close
                                 logger.critical(f"[GOD MODE] ESCAPE: Attempted recovery {stats.recovery_attempt_count}x in {time_since_last_attempt:.0f}s but cap-blocked. Force-closing bucket.")
                                 stats.exit_reason = "FORCE_CLOSE_ESCAPE"
                                 signal = False  # Don't open hedge, just exit
                                 return False
                     ...
                     signal = True
                     stats.recovery_attempt_count += 1
                     stats.last_recovery_attempt_time = time.time()
                     ...
                     if time.time() - self._last_emergency_log > 60:
                         logger.warning(f"[GOD MODE] EMERGENCY TRIGGER: Deficit ${deficit:.2f} > 30% Equity. Forcing Recovery.")
        ...
        # [LOG OPTIMIZATION] Moved "Executing" log to after Constitution Check to prevent spam
        ...
        # [GOD MODE] Request emergency buffer allowance
        # [GOD MODE] Request emergency buffer allowance
        approved, reason = self.trade_authority.check_constitution(broker, symbol, recovery_volume, "OPEN", is_god_mode=True)
        if not approved:
            # [LOG OPTIMIZATION] Rate limit this warning (1 per minute per symbol)
            if not hasattr(self, '_god_mode_spam_filter'): self._god_mode_spam_filter = {}
            last_err = self._god_mode_spam_filter.get(symbol, 0)
            if (time.time() - last_err) > 60:
                logger.warning(f"⚖️ [GOD MODE] BLOCKED by Supreme Court: {reason}")
                self._god_mode_spam_filter[symbol] = time.time()
            return False
            
        # [SUCCESS] Log execution now that it's approved
        logger.info(f"[GOD MODE] Executing Liquidity Recovery: {action} {recovery_volume} lots @ {price}")
    ...
    def _set_position_state(self, bucket_id: str, new_state: PositionState) -> None:
        ...
        with self._state_transition_lock:
            ...
            valid_transitions = {
                ...
                PositionState.PENDING_CLOSE: [PositionState.CLOSED, PositionState.BUCKET_ACTIVE, PositionState.SINGLE_ACTIVE, PositionState.PENDING_CLOSE],  # Allow idempotent
            }
    ...
    def update_positions(self, broker_positions: List[Dict]) -> None:
        ...
        new_positions = {}
        # Consolidate broker positions by ticket
        current_tickets = set()
        for pos_data in broker_positions:
            # Safely extract ticket
            if isinstance(pos_data, dict):
                ticket = pos_data['ticket']
            else:
                ticket = pos_data.ticket
            
            # [CHRONOS PROTOCOL] Skip Toxic Assets held by Bad Bank
            if self.bad_bank.is_toxic(ticket):
                # Do NOT track this position. It belongs to the Bad Bank.
                continue
                
            current_tickets.add(ticket)
            
            # [FIX] Process ALL positions (both new and existing)
            # The old logic only processed NEW positions, causing existing ones to be dropped
            
            # Ghost check (only for new positions)
            if ticket not in self.active_positions and ticket in self._ghost_tickets:
                continue
        ...
        with self._lock:
            # [ROBUSTNESS FIX] Prevent state wipe if parsing failed
            # If we received positions from broker but failed to parse ANY of them,
            # do NOT wipe our existing active_positions. This protects against
            # transient errors, malformed data, or race conditions.
            if not new_positions and broker_positions:
                 logger.warning(f"[SAFETY] update_positions: Input has {len(broker_positions)} items but result is empty. Aborting update to protect state.")
                 return
            ...
            # [CLEANUP] Remove stale tickets from buckets
            # If a position is closed in MT5, it disappears from active_positions.
            # We must also remove it from the bucket stats to prevent "Ghost Trades".
            for bucket_id, stats in list(self.bucket_stats.items()):
                if stats.closed:
                    continue
                
                original_count = len(stats.positions)
                # Keep only tickets that exist in the new_positions map
                stats.positions = [t for t in stats.positions if t in new_positions]
                
                if len(stats.positions) < original_count:
                    removed_count = original_count - len(stats.positions)
                    logger.info(f"[CLEANUP] Removed {removed_count} stale positions from Bucket {bucket_id}")
                    stats.last_update = time.time()
                    self._save_state()
                
                # If bucket becomes empty after cleanup, close it automatically
                if not stats.positions:
                    logger.info(f"[CLEANUP] Bucket {bucket_id} is now empty. Auto-closing.")
                    stats.closed = True
                    self.closed_buckets.add(bucket_id)
    ...
    def find_bucket_by_symbol(self, symbol: str) -> Optional[str]:
        """
        Find the active bucket identifier for a given symbol.
        Returns the first found active bucket or None.
        """
        with self._lock:
            for bucket_id, stats in self.bucket_stats.items():
                 if stats.closed:
                     continue
                 if stats.state == PositionState.CLOSED:
                     continue
                 
                 # Check positions
                 for ticket in stats.positions:
                     if ticket in self.active_positions:
                         if self.active_positions[ticket].symbol == symbol:
                             return bucket_id
        return None
    ...
    def get_calibrated_profit_buffer(self, symbol: str, total_volume: float, base_buffer: float) -> float:
        """
        Get calibrated profit buffer - delegates to the main implementation.
        This wrapper prevents duplicate method definition.
        
        CRITICAL: This buffer protects against spread/slippage losses on close execution.
        Uses P95 slippage calibration system when available, falls back to heuristic.
        """
        # Call the main implementation (defined earlier at line 267)
        return self._calibrated_profit_buffer(symbol, total_volume, base_buffer)
    ...
    def should_close_bucket(self, bucket_id: str, ppo_guardian, market_data=None) -> Tuple[bool, float]:
        ...
        try:
            with self._lock:
                if bucket_id not in self.bucket_stats:
                    logger.debug(f"[TP_CHECK_SKIP] Bucket {bucket_id} not in bucket_stats")
                    return False, 0.0
            ...
            if stats.closed:
                logger.debug(f"[TP_CHECK_SKIP] Bucket {bucket_id} already closed")
                return False, 0.0

            positions = [self.active_positions[t] for t in stats.positions if t in self.active_positions]
            
            if not positions:
                logger.debug(f"[TP_CHECK_SKIP] Bucket {bucket_id} has no active active positions")
                return False, 0.0
            ...
            if target_pips > 0 and current_pips > 0 and current_pips > (target_pips * 0.8):
                ...
                # [DISABLED] Ghost Protocol was too aggressive and caused premature exits
            # Re-enable with stricter thresholds if needed (current: <-0.2 and >70, too sensitive)
            # if first_pos.is_buy:
            #     if (obi is not None and obi < -0.3) or (rsi is not None and rsi > 85):
            #         ghost_exit = True
            # elif first_pos.is_sell:
            #     if (obi is not None and obi > 0.3) or (rsi is not None and rsi < 15):
            #         ghost_exit = True
            ...
            if first_pos.entry_tp_pips > 0:
                ...
                if atr_pips > 0:
                    scalping_tp_pips = atr_pips * 0.5  # Increased from 0.3 to 0.5 (need more distance to prevent instant closes)
                    scalping_sl_pips = atr_pips * 1.0  # 1.0 ATR stop for scalping
                else:
                    # If ATR is unavailable, use conservative minimum to prevent instant closes
                    scalping_tp_pips = 25.0  # Minimum 25 pips (was 0.0, causing immediate closes)
                    scalping_sl_pips = 0.0
            ...
            # [CRITICAL FIX] Enforce minimum TP to prevent instant closes from dynamic calculation
            final_tp_pips = max(final_tp_pips, 15.0)  # At least 15 pips no matter what
            ...
            is_survival_mode = len(positions) >= 4  # [REVERTED] Was 10, caused single positions to lose buffer protection
            ...
            if len(positions) == 1:
                ...
                if first_pos.is_buy:
                    ...
                    # [VERBOSE] Log at debug level only - suppress in normal operation
                    logger.debug(f"[TP CHECK] {first_pos.symbol} BUY | Price: {exit_price:.2f} | Entry: {entry_price:.2f} | Profit: {profit_pips:.1f} pips | Target: {final_tp_pips:.1f} pips")
                    ...
                    # [VERBOSE] Log at debug level only - suppress in normal operation
                    logger.debug(f"[TP CHECK] {first_pos.symbol} SELL | Price: {exit_price:.2f} | Entry: {entry_price:.2f} | Profit: {profit_pips:.1f} pips | Target: {final_tp_pips:.1f} pips")
                ...
                if not profit_exit and final_tp_pips > 0:
                    try:
                        ...
                        if net_pnl >= target_profit_usd:
                            logger.debug(f"[TP FALLBACK] Broker profit target reached: ${net_pnl:.2f} >= ${target_profit_usd:.2f}")
                            profit_exit = True
                    except Exception as e:
                ...
                # [SURVIVAL DYNAMIC] PURE LIVE MARKET LOGIC
                # Floor = Live Spread Cost * 2.0 (Cover spread twice)
                # No fixed dollar values.
                
                # 1. Determine Pip Value USD (Approx)
                pip_val_usd = 10.0 # Standard FX
                if "XAU" in first_pos.symbol or "GOLD" in first_pos.symbol or "JPY" in first_pos.symbol:
                     pip_val_usd = 1.0 # Gold/JPY (Multiplier 100, Contract 100 -> $1/pip)
                
                # 2. Get Live Spread in PIPs
                current_spread_pips = 2.0 # Fallback default
                if spread_points is not None and point > 0:
                     # spread_points is raw price diff (e.g. 0.20)
                     # multiplier is 100 for gold
                     current_spread_pips = float(spread_points) * (1.0 / point) / 10 # Convert points to pips
                     if "XAU" in first_pos.symbol:
                          current_spread_pips = float(spread_points) * 100 # Direct calc logic
                
                # 3. Calculate Real Friction Cost
                # Cost = Volume * SpreadPips * PipValue
                friction_cost = total_volume * current_spread_pips * pip_val_usd
                
                # 4. Target = Friction * 2.0 (Safety Factor)
                # If spread is huge (News), friction is huge -> Target adjusts up.
                # If spread is tiny, friction is tiny -> Target adjusts down.
                dynamic_floor = max(friction_cost * 2.0, total_volume * 1.0) # Absolute min 1 pip profit

                if num_trades == 2:
                    base_50 = base_target_usd * 0.5
                    target_profit_usd = max(base_50, dynamic_floor)
                    logger.debug(f"[SURVIVAL] 2 Trades. Target 50%: ${target_profit_usd:.2f} (Live Spread Cost: ${friction_cost:.2f})")
                elif num_trades >= 3:
                    # Survival Mode: 100% Market Driven
                    target_profit_usd = dynamic_floor
                    logger.debug(f"[SURVIVAL] 3+ Trades. Target Dynamic: ${target_profit_usd:.2f} (Live Spread Cost: ${friction_cost:.2f} * 2.0)")
                else:
                ...
                if len(positions) >= 10:  # Increased from 4 to 10
                    if duration_minutes > TimeThresholds.DEEP_HEDGE_ESCAPE_MINUTES:
                        effective_target = TimeThresholds.DEEP_HEDGE_ESCAPE_TARGET  # Escape
                        status_msg = f" [ESCAPE: Deep Hedge > {TimeThresholds.DEEP_HEDGE_ESCAPE_MINUTES}m]"
                    elif duration_minutes > TimeThresholds.DEEP_HEDGE_DECAY_MINUTES:
                        effective_target = target_profit_usd * TimeThresholds.DEEP_HEDGE_DECAY_MULTIPLIER  # Decay
                        status_msg = f" [DECAY: Deep Hedge > {TimeThresholds.DEEP_HEDGE_DECAY_MINUTES}m]"
                elif len(positions) >= TimeThresholds.LOCAL_HEDGE_CAP:
                    if duration_minutes > TimeThresholds.MED_HEDGE_ESCAPE_MINUTES:
                        effective_target = TimeThresholds.MED_HEDGE_ESCAPE_TARGET
                        status_msg = f" [ESCAPE: Med Hedge > {TimeThresholds.MED_HEDGE_ESCAPE_MINUTES}m]"
                
                # --- [INTELLIGENCE] PROFIT RATCHET LOGIC ---
                # [DISABLED] Ratchet logic was causing premature loss-closing on trades.
                # Root cause: Ratchet would close at $0.50-$1.0 profit, but spread/slippage
                # would flip the actual close price to a loss ($-0.50 to $-30 loss).
                # 
                # Original logic:
                # - Rule A: Peak $2.20 -> Drop to $1.05 -> CLOSE (but spread = -$0.25 loss)
                # - Rule B: Peak $1.50 -> Drop to $0.50 -> CLOSE (but spread = -$1.50 loss)
                # 
                # FIX: Rely on main profit_exit (net_pnl >= effective_target) only.
                # This prevents micro-losses and respects the dynamic target calculation.
                
                # 1. Update High Water Mark (for future analysis, disabled for now)
                ...
                # 2. Ratchet Conditions (ALL DISABLED TO PREVENT LOSS CLOSES)
                ratchet_exit = False
                ratchet_reason = ""
                
                # [DISABLED] Rule A: The "Don't Be Greedy" Lock (80% -> 50%)
                # if current_high >= (effective_target * 0.8) and net_pnl <= (effective_target * 0.5):
                #     if net_pnl > 1.0:  # Too low - spreads exceed $1 easily on XAUUSD
                #         ratchet_exit = True
                #         ratchet_reason = f"[RATCHET PROTECT] Peak ${current_high:.2f} -> Now ${net_pnl:.2f}"
                #         status_msg += " [RATCHET]"

                # [DISABLED] Rule B: The "Breakeven Assist" (50% -> 10%)
                # if current_high >= (effective_target * 0.5) and net_pnl <= (effective_target * 0.1):
                #     min_profit_buffer = 2.0 if volatility_ratio > 1.5 else 0.50  # Still too low for execution
                #     if net_pnl > min_profit_buffer:
                #         ratchet_exit = True
                #         ratchet_reason = f"[BE DEFENSE] Peak ${current_high:.2f} -> Now ${net_pnl:.2f}"
                #         status_msg += " [DEFENSE]"

                profit_exit = (net_pnl >= effective_target)  # Ratchet logic removed
            ...
            # 
            # CRITICAL FIX: Buffer should NOT exceed the actual profit!
            # If net_pnl > $5.00, don't block exits requiring $2.00 buffer.
            ...
            if should_close and not (stop_loss_exit or (emergency_exit and allow_emergency_close_loss)):
                ...
                # CRITICAL FIX: Never let buffer exceed 50% of current profit
                # Otherwise profitable trades get stuck and never close
                # Example: $11.54 profit with $2.00 buffer should close, not wait for $13.54
                if net_pnl > 0:
                    max_acceptable_buffer = net_pnl * 0.5  # Max buffer is 50% of profit
                    min_profit_buffer = min(min_profit_buffer, max_acceptable_buffer)
                ...
                if net_pnl < min_profit_buffer:
                    ...
                    logger.debug(
                        f"[VETO] Exit blocked for {bucket_id}: Net ${net_pnl:.2f} < Buffer ${min_profit_buffer:.2f}. Waiting for recovery."
                    )
                    should_close = False
                    final_confidence = 0.0

            # [CRITICAL SAFEGUARD] Absolute Loss Prevention
            # Even if all conditions say "close", NEVER close if net_pnl is negative or too close to spread cost
            # This prevents the "marginally profitable" trades from flipping to loss at execution
            if should_close and net_pnl < 0:
                exit_reason_list = []
                if time_exit:
                    exit_reason_list.append('TIME')
                if profit_exit:
                    exit_reason_list.append('PROFIT')
                if stop_loss_exit:
                    exit_reason_list.append('STOPLOSS')
                if ai_exit:
                    exit_reason_list.append('AI')
                if wick_exit:
                    exit_reason_list.append('WICK')
                if emergency_exit:
                    exit_reason_list.append('EMERGENCY')
                if spread_cost_exit:
                    exit_reason_list.append('SPREAD')
                
                exit_reason = ', '.join(exit_reason_list) if exit_reason_list else 'UNKNOWN'
                
                logger.warning(
                    f"[LOSS PREVENTION] Close rejected for {bucket_id}: Net PnL is NEGATIVE (${net_pnl:.2f}). "
                    f"Exit reason(s): {exit_reason}. Holding to recovery."
                )
                should_close = False
                final_confidence = 0.0
            ...
            logger.error(f"Recursion depth exceeded evaluating bucket {bucket_id} exit: {re}. Returning False (safe hold)")
            return False, 0.0
        except Exception as e:
    ...
    def offload_bucket_to_bad_bank(self, bucket_id: str) -> bool:
        """
        [CHRONOS PROTOCOL] Offload a frozen bucket to the Bad Bank.
        
        This removes the bucket and its positions from active management,
        effectively "warehousing" the debt so trading can resume cleanly.
        """
        with self._lock:
            if bucket_id not in self.bucket_stats:
                logger.error(f"[CHRONOS] Cannot offload unknown bucket {bucket_id}")
                return False
                
            stats = self.bucket_stats[bucket_id]
            positions_to_offload = []
            
            # Gather position objects
            for ticket in stats.positions:
                if ticket in self.active_positions:
                    positions_to_offload.append(self.active_positions[ticket])
                else:
                    logger.warning(f"[CHRONOS] Position {ticket} missing from active memory during offload")
            
            if not positions_to_offload:
                logger.error(f"[CHRONOS] No positions found to offload for {bucket_id}")
                return False
            
            # 1. Register with Bad Bank
            self.bad_bank.register_toxic_asset(bucket_id, positions_to_offload)
            
            # 2. Purge from PositionManager Memory
            for p in positions_to_offload:
                self.active_positions.pop(p.ticket, None)
                self.active_learning_trades.pop(p.ticket, None)
                
            # 3. Remove Bucket Stats
            del self.bucket_stats[bucket_id]
            
            # 4. Persist State
            self._save_state()
            
            logger.critical(f"⏳ [CHRONOS] Bucket {bucket_id} OFFLOADED to Bad Bank. Partition cleared.")
            return True
    ...
    async def close_bucket_positions(self, broker, bucket_id: str, symbol: str, trace: dict = None, ppo_guardian=None) -> bool:
        ...
        Args:
            ...
            symbol: Trading symbol
            trace: Dict with {"reason": str, "confidence": float, "emergency": bool}
        ...
        with self._lock:
            ...
            # [FIX] Apply trace dict to set exit_reason and mark emergency
            if trace and isinstance(trace, dict):
                reason = trace.get("reason", "UNKNOWN")
                is_emergency_from_trace = trace.get("emergency", False)
                if is_emergency_from_trace:
                    stats.exit_reason = f"EMERGENCY_{reason}"
                else:
        ...
        # [FIX] Use progressive scaling, not multiplicative
        position_count = len(positions)
        if position_count <= 2:
            base_buffer = ProfitBuffer.BUFFER_1_2_POSITIONS  # Single + 1 hedge: minimal buffer
        elif position_count <= 4:
            base_buffer = ProfitBuffer.BUFFER_3_4_POSITIONS  # 2-3 hedges: standard buffer
        elif position_count <= 8:
            base_buffer = ProfitBuffer.BUFFER_5_8_POSITIONS  # 4-7 hedges: moderate buffer
        else:
            base_buffer = ProfitBuffer.BUFFER_9PLUS_POSITIONS  # 8+ hedges: larger buffer (emergency mode)
        
        # [FIX] Override for emergency exits - allow closing with loss
        is_emergency_from_trace = False
        if stats.exit_reason and "EMERGENCY" in (stats.exit_reason or ""):
            base_buffer = ProfitBuffer.EMERGENCY_EXIT_BUFFER  # Nearly zero buffer for emergency closes
            is_emergency_from_trace = True
        ...
        allow_emergency_close_loss = str(os.getenv("AETHER_ALLOW_EMERGENCY_CLOSE_LOSS", "0")).strip().lower() in (
            ...
            "yes",
            "on",
        ) or is_emergency_from_trace  # [FIX] Include emergency from trace
        ...
        if live_net_pnl < min_profit_buffer and not (allow_emergency_close_loss and is_emergency_close):
            ...
            # [FIX] Do NOT rollback state - leave PENDING_CLOSE to prevent repeated close attempts
            # This breaks the infinite loop by preventing immediate re-entry
            # State will naturally progress when positions close or are taken over by hedge logic
            return False
        ...
        if str(os.getenv("AETHER_SLIPPAGE_CALIBRATION_LOG", "1")).strip().lower() in ("1", "true", "yes", "on"):
            sc = self._get_slippage_sample_count(symbol)
            p95pl = self._get_slippage_p95_per_lot_usd(symbol)
            logger.debug(f"[SLIPPAGE] Calibration: samples={sc}, P95/lot=${p95pl:.2f}")

        # [REPORTING] Pre-execution Analysis
        total_volume = sum(pos.volume for pos in positions)
        total_pnl, _, _, _ = self.calculate_net_pnl(positions)
        ...
        # [ATOMIC BATCH CLOSE] Prepare all positions for simultaneous close
        # Instead of sequential CLOSE_BY + remaining + retry, we batch everything
        
        try:
            buys = [p for p in positions if p.type == 0]
            sells = [p for p in positions if p.type == 1]
            
            # Strategy: Try CLOSE_BY for hedged pairs, but prepare ALL in one go
            # This minimizes latency between closes
            
            # Sort by volume for optimal matching
            buys.sort(key=lambda x: x.volume, reverse=True)
            sells.sort(key=lambda x: x.volume, reverse=True)
            
            # Prepare close operations (don't execute yet)
            close_operations = []  # List of (type, position(s)) tuples
            matched_tickets = set()
            
            # Match buys vs sells for CLOSE_BY
            buys_copy = list(buys)
            sells_copy = list(sells)
            
            while buys_copy and sells_copy:
                buy = buys_copy[0]
                sell = sells_copy[0]
                
                match_vol = min(buy.volume, sell.volume)
                match_vol = round(match_vol, 2)
                
                if match_vol < 0.01:
                    break
                
                # Prepare CLOSE_BY operation
                close_operations.append(('CLOSE_BY', buy, sell, match_vol))
                matched_tickets.add(buy.ticket)
                matched_tickets.add(sell.ticket)
                
                # Update volumes
                buy.volume = round(buy.volume - match_vol, 2)
                sell.volume = round(sell.volume - match_vol, 2)
            ...
            # Collect remaining unmatched positions
            remaining = [p for p in positions if p.ticket not in matched_tickets and p.volume >= 0.01]
            
            # [ATOMIC EXECUTION] Execute all closes simultaneously
            logger.info(f"[ATOMIC CLOSE] Batch closing: {len(close_operations)} hedged pairs + {len(remaining)} unmatched")
            
            close_results = {}
            
            # Execute CLOSE_BY operations (these are fast, no async needed)
            for op in close_operations:
                if op[0] == 'CLOSE_BY':
                    _, buy, sell, match_vol = op
                    logger.debug(f"[CLOSE_BY] {buy.ticket} vs {sell.ticket} ({match_vol} lots)")
                    res = broker.close_hedge_by_ticket(buy.ticket, sell.ticket, symbol, match_vol)
                    
                    if res.get('retcode') == mt5.TRADE_RETCODE_DONE:
                        close_results[buy.ticket] = res
                        close_results[sell.ticket] = res
                    else:
                        # If CLOSE_BY fails, add to remaining for normal close
                        logger.warning(f"[CLOSE_BY] Failed, falling back to normal close")
                        remaining.append(buy)
                        remaining.append(sell)
            
            # [CRITICAL] Close all remaining positions in ONE async batch
            if remaining:
                logger.info(f"[BATCH CLOSE] Closing {len(remaining)} positions simultaneously...")
                rem_res = await broker.close_positions(remaining, trace=trace)
                close_results.update(rem_res)
                
        except Exception as e:
            logger.error(f"[ATOMIC CLOSE] Error: {e}")
            # Fallback: close all normally
            try:
                close_results = await broker.close_positions(positions, trace=trace)
            except:
                pass
        ...
        # [CHRONOS PROTOCOL] Tithe Collection (Debt Repayment)
        if total_pnl > 0 and self.bad_bank.toxic_assets:
             TITHE_RATE = 0.50 # 50% of profits go to debt repayment
             tithe_amount = total_pnl * TITHE_RATE
             self.bad_bank.deposit_tithe(tithe_amount, symbol)
             
             # Attempt to pay down debt immediately
             logger.info(f"💰 [CHRONOS] Tithe Collected: ${tithe_amount:.2f} (50% of profit)")
             # Fire and forget debt reduction (async)
             try:
                await self.bad_bank.attempt_debt_reduction(broker)
             except Exception as e:
                logger.error(f"[CHRONOS] Debt reduction failed: {e}")

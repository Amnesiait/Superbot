...
from .ai_core.contrastive_fusion import ContrastiveFusion
from .utils.trader_dashboard import get_dashboard
from .ai_core.trap_hunter import TrapHunter
from .ai_core.wick_intelligence import get_wick_intelligence
from .ai_core.liquidity_mapper import LiquidityMapper
from .ai_core.global_brain import GlobalBrain
from .ai_core.hybrid_hedge_intelligence import HybridHedgeIntelligence
from .ai_core.multi_horizon_predictor import MultiHorizonPredictor
...
class TradingEngine:
    ...
    def __init__(self, config: TradingConfig, broker_adapter, market_data, position_manager, risk_manager, db_manager: Optional[AsyncDatabaseManager] = None, ppo_guardian=None, global_brain=None, tick_analyzer=None):
        ...
        self.liquidity_mapper = LiquidityMapper() # [LIQUIDITY WALLS] Initialize Liquidity Mapper
        self.fusion = ContrastiveFusion() # [THE JUDGE] Initialize Contrastive Fusion
        self.fusion = ContrastiveFusion() # [THE JUDGE] Initialize Contrastive Fusion
        
        # [GOD MODE] Macro-Economics (Use shared instance if provided)
        if global_brain:
             self.global_brain = global_brain
        else:
             self.global_brain = GlobalBrain(self.market_data)
             
        self.multi_horizon = MultiHorizonPredictor() # [FUTURE SIGHT] Multi-Timeframe Oracle
        self.architect = Architect(self.broker) # [SPATIAL AWARENESS] H1 Structure
        # Upgrade Hedge Intelligence to Hybrid
        self.hedge_intelligence = HybridHedgeIntelligence() # [V2.0] Hybrid Risk Management
        ...
        # [DUPLICATE PREVENTION] Atomic Execution Locks
    ...
    def validate_market_conditions(self, symbol: str, tick: Dict) -> Tuple[bool, str]:
        """
        [SUPERBOT] Validación mejorada con Veto de Salud del Spread.
        Mantiene la compatibilidad con el MarketData original.
        """
        if not tick or 'bid' not in tick or 'ask' not in tick:
            return False, "Invalid tick data"

        # --- INYECCIÓN SUPERBOT PASO 1.3 ---
        # Calculamos el spread bruto actual
        point_factor = (0.01 if "XAU" in symbol or "JPY" in symbol else 0.0001)
        current_raw_spread = abs(tick['ask'] - tick['bid']) / point_factor
        
        # Consultamos al RiskManager si el spread es saludable
        if not self.risk_manager.is_spread_healthy(float(current_raw_spread)):
            return False, f"VETO: Spread Unhealthy/Illiquid ({current_raw_spread:.1f})"
        # ----------------------------------

        # Validación de blackout de noticias (mantiene tu lógica original)
        if self.news_calendar.is_blackout_period(symbol):
            return False, "News Blackout (Event Horizon)"

        # Validación de spread máximo configurado
        multiplier = 100 if ("JPY" in symbol or "XAU" in symbol) else 10000
        ...
        # Llamada al validador de market_data original para el resto de filtros
    ...
    def _validate_physics_conditions(self, trade_direction: str, regime: str, metrics: Dict, tick: Dict) -> Tuple[bool, str]:
        """
        [HIGHEST INTELLIGENCE] The Physicist
        Validates market conditions using Physical Dynamics (Reynolds Number, Turbulence).
        """
        physics = metrics.get('physics', {})
        chemistry = metrics.get('chemistry', {})
        
        # 1. REYNOLDS NUMBER (Turbulence)
        # Re > 5000 implies Chaotic/Turbulent flow (Random Walk). Technical Analysis breaks down.
        re = physics.get('reynolds_number', 0)
        # Tuning: 5000 is a conservative threshold for "Chaos".
        if re > 5000:
            return False, f"Extreme Turbulence (Re={re:.0f}). Market is Chaos."
            
        # 2. VPIN (Toxic Flow)
        # High VPIN means Informed Traders are poisoning the well. High risk of manipulation.
        vpin = chemistry.get('vpin', 0)
        if vpin > 0.6:
            return False, f"Toxic Flow Detected (VPIN={vpin:.2f}). Manipulation Risk."
            
        # 3. NAVIER-STOKES FORCE (Momentum)
        # Ensure we are not swimming against a massive current
        force = physics.get('navier_stokes_force', 0)
        
        if trade_direction == "BUY" and force < -100: # Strong Downward Force
             return False, f"Fighting Downward Momentum Force ({force:.1f})"
        if trade_direction == "SELL" and force > 100: # Strong Upward Force
             return False, f"Fighting Upward Momentum Force ({force:.1f})"
             
        return True, "Physics Nominal"
    ...
    def validate_trade_entry(self, signal: TradeSignal, lot_size: float,
        ...
        if not is_recovery_trade:
            ...
            if existing_positions:
                # Convert signal direction to MT5 type
                # TradeAction.BUY -> 0 (ORDER_TYPE_BUY)
                # TradeAction.SELL -> 1 (ORDER_TYPE_SELL)
                target_type = 0 if signal.action == TradeAction.BUY else 1
        ...
        # [1] PHYSICS CHECK (Reynolds Number / Turbulence)
        # Verify market is not in "Chaos Mode" (Extreme Turbulence)
        # [UPGRADE] Use Combined Analysis (Physics + Chemistry)
        combined_metrics = self.tick_analyzer.get_combined_analysis(tick)
        pressure_metrics = combined_metrics # Backwards compat
        physics_data = combined_metrics.get('physics', {})
        
        # Get regime if available
        regime = "RANGE"
        if hasattr(self.market_data, 'regime_detector'):
             regime = self.market_data.regime_detector.current_regime
        
        physics_ok, physics_reason = self._validate_physics_conditions(
            signal.action.value, regime, combined_metrics, tick
        )
        if not physics_ok:
            logger.warning(f"[PHYSICS] Entry Blocked: {physics_reason}")
            return False, f"Physics Violation: {physics_reason}"

        # [2] LIQUIDITY WALLS (LiquidityMapper)
        # Verify we are not buying into Resistance or Selling into Support
        # Need history for mapper
        recent_candles = []
        if hasattr(self.market_data, 'candles'):
             try:
                 recent_candles = self.market_data.candles.get_history(symbol, timeframe="M1", count=100)
             except Exception:
                 pass
        
        if recent_candles:
            liq_map = self.liquidity_mapper.map_liquidity(recent_candles, tick['bid'])
            avoid, liq_reason = self.liquidity_mapper.should_avoid_entry(
                tick['bid'], signal.action.value, liq_map
            )
            if avoid:
                return False, f"Liquidity Wall: {liq_reason}"

        # [2.5] SPATIAL AWARENESS (The Architect)
        # Check Macro Structure (H1 Walls) to prevent hitting ceilings/floors
        if self.architect:
            structure = self.architect.get_market_structure(signal.symbol)
            if structure:
                status = structure.get('status', 'CLEAR')
                if status == 'BLOCKED_UP' and signal.action.value == "BUY":
                    # Check if we have "Battering Ram" momentum (Strong Force)
                    force = combined_metrics.get('physics', {}).get('navier_stokes_force', 0)
                    if force < 150: # If force is not extreme, respect the wall
                         logger.warning(f"[ARCHITECT] Entry Blocked: Hitting H1 Resistance (Room: {structure['room_up']:.4f})")
                         return False, f"Structural Roof (Architect): {structure['room_up']:.4f}"
                    else:
                         logger.info(f"[ARCHITECT] ⚠️ Battering Ram Mode: Breaking H1 Resistance with Force {force:.0f}")

                elif status == 'BLOCKED_DOWN' and signal.action.value == "SELL":
                    force = combined_metrics.get('physics', {}).get('navier_stokes_force', 0)
                    if force > -150:
                         logger.warning(f"[ARCHITECT] Entry Blocked: Hitting H1 Support (Room: {structure['room_down']:.4f})")
                         return False, f"Structural Floor (Architect): {structure['room_down']:.4f}"
                    else:
                         logger.info(f"[ARCHITECT] ⚠️ Battering Ram Mode: Breaking H1 Support with Force {force:.0f}")

        # [3] THE JUDGE (Contrastive Fusion)
        # Verify Coherence between Price (Signal), Volume (Pressure), and Order Book (OBI)
        # 1. Price Signal: Oracle Confidence (Normalized direction)
        sig_val = signal.confidence if signal.action.value == "BUY" else -signal.confidence
        
        # 2. Pressure Signal: Normalized Pressure
        raw_pressure = pressure_metrics.get('pressure_score', 0)
        pressure_val = max(-1.0, min(1.0, raw_pressure / 50.0)) # Normalize -50..50 to -1..1
        
        # 3. Liquidity/OBI Signal: From OBI Validator (calculated above? No, we need to fetch it)
        # For now, we use a simplified OBI proxy from TickPressure's 'order_flow_imbalance'
        # which is roughly -1 to 1.
        obi_val = combined_metrics.get('order_flow_imbalance', 0.0)
        
        fusion_inputs = {
            "tick_velocity": pressure_val, # Use Pressure as velocity proxy in this context
            "candle_pattern": sig_val,     # Use Oracle signal as 'Price' input
            "order_book": obi_val
        }
        
        coherence = self.fusion.compute_coherence(fusion_inputs)
        
        # [INTELLIGENT EXCEPTION] Extreme RSI Bypass
        # Overbought (RSI > 70) and Oversold (RSI < 30) are HIGH-PROBABILITY reversal setups
        # that don't require perfect coherence between other signals.
        # These are STRUCTURAL price extremes that should NOT be blocked by low coherence.
        rsi_extreme_exception = False
        if hasattr(signal, 'metadata') and signal.metadata:
            rsi_val = signal.metadata.get('rsi', None)
            if rsi_val is not None:
                rsi_val = float(rsi_val)
                # Overbought (RSI > 70) selling or Oversold (RSI < 30) buying
                is_overbought_sell = signal.action == TradeAction.SELL and rsi_val > 70
                is_oversold_buy = signal.action == TradeAction.BUY and rsi_val < 30
                
                if is_overbought_sell or is_oversold_buy:
                    rsi_extreme_exception = True
                    logger.info(
                        f"[FUSION EXCEPTION] RSI Extreme Setup Detected: "
                        f"{'OVERBOUGHT' if is_overbought_sell else 'OVERSOLD'} (RSI {rsi_val:.1f}). "
                        f"Bypassing coherence check (was {coherence:.2f})."
                    )
        
        # Allow entry if Coherence > 0.5 (Pass)
        # OR if RSI is at extreme levels (high-probability reversals)
        if coherence < 0.2 and not rsi_extreme_exception: # Very strict block on extremely low coherence
             logger.warning(f"[FUSION] Entry Blocked: Low Coherence ({coherence:.2f}) Inputs: {fusion_inputs}")
             return False, f"Signal Incoherent (The Judge): {coherence:.2f}"

        # [4] GOD MODE (Global Brain)
        # Check Macro-Economic Drivers (DXY, Yields).
        # We generally DO NOT BLOCK scalps based on Macro (too slow), but we check for CATASTROPHIC divergence.
        macro_bias = self.global_brain.get_bias() # -1.0 to 1.0 (Positive = Bullish Gold)
        
        # If Signal is BUY and Macro is Strong Bearish (-0.8) -> Dangerous
        if signal.action.value == "BUY" and macro_bias < -0.8:
            logger.warning(f"[GLOBAL_BRAIN] DANGEROUS: Buying against Strong Macro Headwinds (Bias: {macro_bias:.2f})")
            # We don't block (user request: continuous trading), but we flag it for sizing reduction
            # This updated logic will be handled by IronShield if we pass it down, or we just log it here.
            
        # [5] FUTURE SIGHT (Multi-Horizon)
        # Predict the next 5 minutes. If we are entering into a predicted Reversal, be careful.
        if recent_candles:
             horizon_pred = self.multi_horizon.predict_all_horizons(recent_candles, tick)
             
             # Consensus Check
             # If Signal is BUY, but Multi-Horizon Consensus is DOWN -> Reversal Imminent?
             if signal.action.value == "BUY" and horizon_pred.consensus_direction == "DOWN":
                 if horizon_pred.should_wait_for_reversal:
                     # [QUANTUM SNIPER] Instead of Blocking, Place Limit Order at predicted bottom
                     predicted_dip_pips = horizon_pred.expected_pips if horizon_pred.expected_pips > 0 else 10.0
                     # Cap dip at 30 pips to avoid missing huge moves, floor at 5 pips
                     dip_pips = max(5.0, min(predicted_dip_pips, 30.0))
                     
                     limit_price = tick['ask'] - (dip_pips * (0.01 if "XAU" in signal.symbol or "JPY" in signal.symbol else 0.0001))
                     
                     logger.info(f"[QUANTUM SNIPER] 🎯 Converting BUY to BUY_LIMIT @ {limit_price:.5f} (Predicted Dip: {dip_pips:.1f} pips)")
                     # Return special instruction tuple: (True, "Reason", "ACTION_OVERRIDE", price)
                     return True, f"Quantum Sniper: Limit Entry @ Dip (-{dip_pips:.1f}p)", "BUY_LIMIT", limit_price

             if signal.action.value == "SELL" and horizon_pred.consensus_direction == "UP":
                 if horizon_pred.should_wait_for_reversal:
                     # [QUANTUM SNIPER] Instead of Blocking, Place Limit Order at predicted peak
                     predicted_rally_pips = horizon_pred.expected_pips if horizon_pred.expected_pips > 0 else 10.0
                     rally_pips = max(5.0, min(predicted_rally_pips, 30.0))
                     
                     limit_price = tick['bid'] + (rally_pips * (0.01 if "XAU" in signal.symbol or "JPY" in signal.symbol else 0.0001))
                     
                     logger.info(f"[QUANTUM SNIPER] 🎯 Converting SELL to SELL_LIMIT @ {limit_price:.5f} (Predicted Rally: {rally_pips:.1f} pips)")
                     return True, f"Quantum Sniper: Limit Entry @ Peak (+{rally_pips:.1f}p)", "SELL_LIMIT", limit_price




        # [WICK INTELLIGENCE] Block trades at wick extremes

        # [WICK INTELLIGENCE] Block trades at wick extremes
        wick_intel = get_wick_intelligence()
        # Note: We need recent candles. If market_data has them, use them.
        # Otherwise, skip or fetch. For speed, we try to access market_data history if available.
        # Reuse candles if available, otherwise fetch
        if not recent_candles and hasattr(self.market_data, 'candles'):
             # Try to get last 20 M1 candles
             try:
                 recent_candles = self.market_data.candles.get_history(symbol, timeframe="M1", count=20)
             except Exception:
                 pass
        
        # Check against current price
        # Normalize signal direction to "BUY" or "SELL"
        direction = signal.action.value
        should_block, block_reason = wick_intel.should_block_trade(
            direction, tick['bid'] if direction == "SELL" else tick['ask'], recent_candles
        )
        
        if should_block:
            logger.warning(f"[WICK] Entry Blocked: {block_reason}")
            return False, f"Wick Protection: {block_reason}"
        ...
        if not is_recovery_trade:
             ...
             if veto:
                 # [CHRONOS PROTOCOL] Intercept Trigger
                 if "CHRONOS_TRIGGER" in veto_reason:
                     logger.critical(f"⏳ [CHRONOS] Intercepted Governor Veto: {veto_reason}")
                     
                     # 1. Identify the Toxic Bucket (Simply finding the largest drawdown bucket)
                     # In Single-Asset mode, we treat the current symbol as the target.
                     toxic_bucket_id = self.position_manager.find_bucket_by_symbol(signal.symbol)
                     
                     if toxic_bucket_id:
                         # 2. Execute Perfect Hedge (Valkyrie Freeze)
                         logger.critical(f"❄️ [VALKYRIE] Executing Perfect Hedge for {toxic_bucket_id}")
                         self.position_manager.execute_perfect_hedge(self.broker, toxic_bucket_id, tick)
                         
                         # 3. Offload to Bad Bank
                         logger.critical(f"🏦 [BAD BANK] Offloading {toxic_bucket_id}...")
                         if self.position_manager.offload_bucket_to_bad_bank(toxic_bucket_id):
                             logger.critical("✅ [CHRONOS] Offload Complete. Resetting Emergency State.")
                             
                             # 4. Reset Governor to allow Fresh Start
                             self._governor.reset_emergency_mode(admin_password="RESET_AETHER_2026")
                             
                             # Allow this trade to proceed as the *First Trade* of the new partition
                             return True, "Chronos Protocol Executed - Fresh Start"
                         else:
                             return False, "Chronos Offload Failed - HALTING"
                     else:
                         return False, "Chronos Triggered but No Active Bucket Found"
    ...
    async def execute_trade_entry(self, signal: TradeSignal, lot_size: float,
                          tick: Dict, strategist, shield, action_override=None, price_override=None) -> Optional[Dict]:
        """
        ...
        Args:
            ...
            shield: IronShield instance
            action_override: Strict override for TradeAction (e.g. "BUY_LIMIT")
            price_override: Strict override for Entry Price
        ...
        try:
            # [AUDIT] Input Validation (Paranoia Check)
            if not tick or 'ask' not in tick or 'bid' not in tick:
                 logger.error(f"[EXECUTION] Invalid tick data provided: {tick}")
                 return None
            if lot_size <= 0:
                 logger.error(f"[EXECUTION] Invalid lot size: {lot_size}")
                 return None

            # [DUPLICATE PREVENTION] Atomic Entry Lock
            # Prevents "Machine Gun" entries if multiple signals fire in < 5s
            last_exec = self._execution_locks.get(signal.symbol, 0.0)
            if time.time() - last_exec < 5.0:
                 logger.debug(f"[EXEC_LOCK] Skipping entry for {signal.symbol}: Cooldown active ({time.time() - last_exec:.1f}s < 5.0s)")
                 return None
            
            # Set lock immediately - blocks any other thread/signal usage
            self._execution_locks[signal.symbol] = time.time()
            ...
            # Determine entry price and order type (Allow Overrides)
            if action_override:
                order_type = action_override
                entry_price = price_override if price_override else tick['ask'] # Fallback
            ...
            # [QUANTUM SNIPER] Specific handling for Limit Orders
            if "LIMIT" in order_type:
                logger.info(f"[QUANTUM SNIPER] Executing PENDING ORDER: {order_type} @ {entry_price:.5f}")
            ...
            # [REMOVED DUPLICATE EXECUTION BLOCK]
            # Flow continues to parameter validation and final execution below.
            if atr_value <= 0.0:
            ...
            # [AUDIT] Dynamic Contract Size from Broker
            contract_size = self.market_data.get_contract_size(signal.symbol)
    ...
    async def process_position_management(self, symbol: str, positions: List[Dict],
        ...
        if not bucket_id:
            ...
            if position_objects:
                ...
                logger.warning(f"[BUCKET] Failed to create bucket - positions not found in active_positions (Ticket mismatch?)")
        else:
        ...
        if not bucket_closed:
            ...
            zone_recovery_executed = self.risk_manager.execute_zone_recovery(
                ...
                hybrid_intelligence=self.hedge_intelligence # [GOD MODE] Connect V2.0 Logic
            )
            if zone_recovery_executed:
                logger.debug(f"[ZONE] RECOVERY EXECUTED for {symbol}")
                ...
                if self.entry_cooldowns is not None:
                     ...
                     logger.debug(f"[PIPELINE COMMIT] Hedge executed -> Locked Initial Entry Cooldown for {symbol}")
                ...
                if all_positions:
                    ...
                    logger.debug(f"[SYNC] Positions updated after hedge. Total: {len(all_positions)}")
            else:
    ...
    async def run_trading_cycle(self, shield, ppo_guardian,
        ...
        try:
            ...
            if not market_ok:
                # [MODIFICACIÓN SUPERBOT] Detectar si la razón es el nuevo Veto de Salud
                is_spread_issue = "Spread too wide" in reason or "VETO" in reason
                
                should_log = False
                if is_spread_issue:
                    if self.last_pause_reason is None or ("Spread too wide" not in self.last_pause_reason and "VETO" not in self.last_pause_reason):
                        should_log = True
                elif reason != self.last_pause_reason:
                    should_log = True

                if should_log:
                    logger.debug(f"[PAUSED] TRADING PAUSED: {reason}")
                    # Si es un VETO de salud, queremos verlo en la terminal aunque sea un tema de spread
                    if "VETO" in reason:
                        print(f">>> [LIQUIDITY ALERT] {reason}", flush=True)
                    elif not is_spread_issue:
            ...
            if self.last_pause_reason is not None:
                logger.debug(f"[RESUMED] TRADING RESUMED: Market conditions normalized.")

                self.last_pause_reason = None
            
            # Cycle running normally - verbose diagnostics disabled to reduce noise
            # print(f">>> [DEBUG] Cycle Running: {symbol}", flush=True)
            ...
            if positions:
                # MANAGEMENT MODE
                # print(f">>> [DEBUG] Processing {len(positions)} existing positions...", flush=True)
            ...
            # Hunting mode - no existing positions
            ...
            if trap_signal.is_trap and trap_signal.suggested_action in ("BUY", "SELL"):
                 ...
                 if trap_signal.counter_confidence >= 0.8:
                     ...
                     # [DUPLICATE FIX] Check entry cooldown (same as main entry logic)
                     import threading
                     if not hasattr(self, '_entry_lock'):
                         self._entry_lock = threading.Lock()
                     
                     acquired = self._entry_lock.acquire(blocking=True, timeout=0.5)
                     if not acquired:
                         logger.warning(f"[PREDATOR] Failed to acquire entry lock - skipping to prevent duplicate")
                         return
                     ...
                     try:
                         ...
                         if time_since_last < cooldown_limit:
                             if time_since_last > 1.0:
                                 logger.info(f"[PREDATOR] Cooldown active ({time_since_last:.1f}s < {cooldown_limit}s) - skipping")
                             return
                         
                         # Execute if approved and not vetoed
                         if not veto and approved:
                              # Reserve entry slot
                              self.entry_cooldowns[symbol] = time.time()
                              logger.info(f"[ENTRY_LOCK] ✅ PREDATOR entry reserved for {symbol}")
            ...
            if oracle:
                ...
                if len(history) >= 60:
                    ...
                    if oracle_confidence > 0.5: # Lower threshold as Fusion is stricter
                        logger.debug(f"[ORACLE] Prediction: {oracle_prediction} ({oracle_confidence:.2f}) | {oracle_result['reason']}")
                    ...
                    logger.debug(f"[ORACLE] Regime: {oracle_regime} | Signal: {oracle_signal}")
            ...
            logger.debug(f"[SUPERVISOR] Market Regime: {regime.name} ({regime.confidence:.2f}) | {regime.description}")
            ...
            elif worker_name == "DEFENSIVE_WORKER":
                # Fallback for CHAOS or DEFENSIVE regimes - use conservative range strategy
                action, confidence, reason = self.range_worker.get_signal(market_context)
                reason = f"[DEFENSIVE] {reason}"

            # [FILTER] Block Zero/Low Confidence Signals Immediately
            # Prevents pollution of downstream logic with "BUY 0%" noise.
            if action != "HOLD" and confidence <= 0.15:
                 logger.debug(f"[FILTER] Signal Ignored: {action} with low confidence {confidence:.2f} ({reason})")
                 action = "HOLD"
            ...
            if action != "HOLD" and confidence > 0.5:
                ...
                if not physics_ok:
                    # [PHYSICS] Soft Block (User Preference: Continuous Trading)
                    # Instead of blocking, we apply a heavy penalty and log a warning.
                    logger.warning(f"[PHYSICS WARNING] {action} flagged: {physics_reason} (Reducing Confidence)")
                    # action = "HOLD" <-- DISABLED
                    confidence = confidence * 0.5 # Apply 50% penalty
                    reason = f"{reason} | [PHYSICS-WARN] {physics_reason}"
            ...
            if action != "HOLD" and confidence > 0.5:
                ...
                try:
                    ...
                    if validation.should_invert and validation.score < 0.20:
                        ...
                        logger.debug(f"[SIGNAL] Waiting for stronger signal (need ≥40% validation)")
            ...
            # [CRITICAL FIX] Double Entry Prevention (Thread-Safe + Async-Safe)
            # Use threading.Lock with proper async handling
            import threading
            if not hasattr(self, '_entry_lock'):
                self._entry_lock = threading.Lock()
            
            # Atomic cooldown check and set (ASYNC-COMPATIBLE)
            acquired = self._entry_lock.acquire(blocking=True, timeout=0.5)
            if not acquired:
                logger.warning(f"[ENTRY_LOCK] Failed to acquire lock for {symbol} - skipping to prevent race")
                return
            ...
            try:
                ...
                if time_since_last < cooldown_limit:
                    ...
                    if time_since_last > 1.0: 
                         logger.info(f"[COOLDOWN] Entry blocked for {symbol}. Last entry was {time_since_last:.1f}s ago (limit: {cooldown_limit}s).")
                    return
                
                # Reserve this entry slot immediately (atomic)
                self.entry_cooldowns[symbol] = time.time()
                logger.info(f"[ENTRY_LOCK] ✅ Entry slot reserved for {symbol}. Cooldown {cooldown_limit}s active.")
            finally:
                self._entry_lock.release()
            ...
            if self.global_brain and ("XAU" in symbol or "GOLD" in symbol):
                ...
                if correlation_signal is not None and getattr(correlation_signal, 'confidence', 0.0) > 0.0:
                    ...
                    if global_bias < -0.5 and signal.action == TradeAction.BUY:
                        brain_penalty = 0.5
                        logger.debug(f"[GLOBAL_BRAIN] CAUTION: Bearish Correlation ({global_bias:.2f}). Reducing lots by 50%.")
                    elif global_bias > 0.5 and signal.action == TradeAction.SELL:
                        brain_penalty = 0.5
                        logger.debug(f"[GLOBAL_BRAIN] CAUTION: Bullish Correlation ({global_bias:.2f}). Reducing lots by 50%.")
            ...
            validation_result = self.validate_trade_entry(signal, lot_size, account_info, tick, is_recovery_trade=False)
            ...
            # [QUANTUM SNIPER] Tuple Unpacking
            if isinstance(validation_result, tuple):
                if len(validation_result) == 2:
                     can_enter, entry_reason = validation_result
                elif len(validation_result) == 4:
                     can_enter, entry_reason, override_action, override_price = validation_result
            else:
                 logger.error(f"[CRITICAL] validate_trade_entry returned unknown type: {type(validation_result)}")
                 return

            if can_enter and override_action:
                 # Apply Quantum Sniper Overrides
                 logger.info(f"[QUANTUM SNIPER] ⚡ OVERRIDE: {signal.action.value} -> {override_action} @ {override_price:.5f}")
                 # Modify signal/tick locally for this execution context
                 # Note: We don't modify the original signal object to avoid side effects elsewhere
                 # But we need to pass these to execute_trade_entry or call broker directly?
                 # Actually, execute_trade_entry uses signal.action. We should probably create a temporary signal copy/mock?
                 # Or better, we manually execute it here to be safe and specific.
                 
                 # Force the signal action to match the override (BUY_LIMIT/SELL_LIMIT support needed in TradeAction?)
                 # TradeAction enum might not support LIMIT. 
                 # Instead, we pass 'order_type_override' to execute_trade_entry if we modify it.
                 # Let's modify execute_trade_entry to accept overrides.
            ...
            if not can_enter:
                ...
                # Suppress repetitive blocking logs (cooldown, position exists) - only log once when signal changes
                    ...
                    logger.debug(f"[TRADE] ENTRY BLOCKED: {entry_reason}")
            ...
            # Pass specific overrides if Quantum Sniper engaged
            result = await self.execute_trade_entry(
                signal, lot_size, tick, None, shield, 
                action_override=override_action, 
                price_override=override_price
            )
            
            if result:
                order_desc = f"{override_action if override_action else signal.action.value}"
                price_desc = f"@{override_price:.5f} (Limit)" if override_price else f"@{tick['ask'] if signal.action == TradeAction.BUY else tick['bid']:.5f}"
                logger.info(f"[TRADE] EXECUTION SUCCESSFUL: {order_desc} {signal.symbol} {lot_size} lots {price_desc}")
    ...
    async def _process_existing_positions(self, symbol: str, tick: Dict, shield, ppo_guardian, oracle=None, pressure_metrics=None) -> bool:
        ...
        if symbol_positions:
            ...
            if positions_managed:
                logger.debug(f"[POSITION] MANAGEMENT EXECUTED for {symbol} - Skipping new entries")
                return True
